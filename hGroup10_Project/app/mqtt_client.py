...
def handle_test(client, raw: bytes) -> None:
    data = validation.parse_json(raw)

    if not isinstance(data, dict):
        return

    # Ignore acknowledgements and failure notifications.
    if data.get("type") != "mqtt_test":
        return

    response = data.get("response")

    # Validate Central's response object.
    if not isinstance(response, dict):
        log.warning("Test request missing response object")
        return

    if (
        response.get("type") != "mqtt_test_response"
        or response.get("team") != config.TEAM_NAME
        or response.get("requestId") != data.get("requestId")
        or response.get("status") != "ok"
    ):
        log.warning("Invalid MQTT test response object")
        return

    # Publish Central's specified response.
    ...
    if result.rc == 0:
        log.info(
            "MQTT test response queued: %s",
            response["requestId"]
        )
    else:
        log.error(
            "MQTT test publish failed: %s",
            result.rc
        )
