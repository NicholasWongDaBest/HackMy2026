...
class NodeSerial:
    ...
    def handle_line(self, raw: bytes) -> None:
        ...
        # The board has shipped in two shapes over the project's life:
        #   {"type":"pump","state":"on",...}   and   {"pump":"on",...}
        # Accept both. Getting this wrong is silent and nasty: the Pi keeps
        # believing the pump is running after the board's safety timeout
        # has already switched it off, and the dashboard lies to a judge.
        state = message.get("state") if message.get("type") == "pump" else None
        if state is None:
            state = message.get("pump")
        if state in {"on", "off"}:
