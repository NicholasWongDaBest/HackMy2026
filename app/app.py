...
from . import config, control, database, viz
...
def dashboard():
    ...
    # Challenge 2: stable-vs-malicious comparison. Temperature is the
    # brief's headline example (888-999), and the probe holds a rock-steady
    # ~24 C, so the contrast reads at a glance.
    chart_type = "temperature"
    band = config.SENSOR_RANGES.get(chart_type, (None, None))
    chart_svg = viz.anomaly_chart_svg(
        database.series_for_type(chart_type, 60),
        database.range_rejections(chart_type, 80),
        chart_type, band=band, unit="degC",
    )
    anomaly_count = database.anomaly_window(5)
