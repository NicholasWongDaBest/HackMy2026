...
# ---------------------------------------------------------------------
# Challenge 2: anomaly detection for the dashboard
# ---------------------------------------------------------------------
_RANGE_RE = re.compile(r"^(\w+) reading (-?\d+(?:\.\d+)?) outside")


def anomaly_window(minutes: int = 5) -> int:
    """How many messages were rejected in the last `minutes`. Drives the
    dashboard warning banner."""
    with local() as conn:
        cur = conn.cursor(dictionary=True)
        cur.execute(
            "SELECT COUNT(*) AS n FROM rejected_messages"
            " WHERE rejected_at >= (NOW() - INTERVAL %s MINUTE)",
            (minutes,),
        )
        return cur.fetchone()["n"]


def series_for_type(sensor_type: str, limit: int = 60):
    """Recent in-range readings for one measurand, oldest first -- the
    stable line on the comparison chart."""
    with local() as conn:
        cur = conn.cursor(dictionary=True)
        cur.execute(
            "SELECT sensor_value, created_at FROM sensor_data"
            " WHERE sensor_type = %s ORDER BY id DESC LIMIT %s",
            (sensor_type, limit),
        )
        rows = cur.fetchall()
    rows.reverse()
...
def range_rejections(sensor_type: str = None, limit: int = 80):
    """Rejections whose reason carries a numeric out-of-range value, parsed
    back out for plotting. Non-numeric rejections (markup, bad JSON) are
    skipped here but still counted by anomaly_window()."""
    with local() as conn:
        cur = conn.cursor(dictionary=True)
        cur.execute(
            "SELECT reason, rejected_at FROM rejected_messages"
            " ORDER BY id DESC LIMIT %s",
            (limit,),
        )
        rows = cur.fetchall()
    out = []
    for r in rows:
        m = _RANGE_RE.match(r.get("reason") or "")
        if not m:
            continue
        stype, val = m.group(1), float(m.group(2))
        if sensor_type and stype != sensor_type:
            continue
        out.append({"sensor_type": stype, "value": val, "rejected_at": r["rejected_at"]})
    out.reverse()
    return out
